...
def get_engine(db_name):
    ...
    """
    resolved_name = get_database_name(db_name)
...
def get_pymysql_connection(db_name):
    ...
    """
    resolved_db_name = get_database_name(db_name)
    pid = os.getpid()
    key = (pid, db_name)

    if key not in _PYMYSQL_CONNECTION_CACHE:
        print(
            f"[get_pymysql_connection] Creating connection for DB: {resolved_db_name}, PID: {pid}"
        )
        _PYMYSQL_CONNECTION_CACHE[key] = create_pymysql_connection(resolved_db_name)
...
def _dispose_all_engines(*args, **kwargs):
    ...
    for pid, engine in _PROCESS_ENGINE_CACHE.items():
        try:
            ...
            with open(f'{os.environ["DBASE_DIR"]}/logs/atexit.log', "a") as f:
                f.write(
                    f"Error Disposing Engine: {str(pid)}, {e} on {datetime.now()}\n"
                )
            pass
    for conn in _PYMYSQL_CONNECTION_CACHE.values():
        try:
            ...
            with open(f'{os.environ["DBASE_DIR"]}/logs/atexit.log', "a") as f:
                f.write(
                    f"Error Closing Connection: {str(pid)}, {e} on {datetime.now()}\n"
                )
            pass
...
def clear_table_data(db: str, table_name: str) -> bool:
    ...
    with engine.connect() as connection:
        ...
        connection.execute(
            text(f"ALTER TABLE {table_name} AUTO_INCREMENT = 1")
...
def drop_SQL_Table_Duplicates(db, sql_table_name):
    ...
    # RE-QUERY WHOLE DATA
    df = query_database(db, sql_table_name, f"SELECT * FROM {db}.{sql_table_name}")

    # DROP DUPLICATES OF WHOLE DATA
    df = df.drop_duplicates()
    range_ = list(
        range(0, len(df), 10000),
    )
...
def create_SQL_connection(database: str = None):
    ...
    resolved_db = get_database_name(database)
...
def get_table_schema(db_name, table_name) -> list[dict[str, str]]:
    ...
    # Resolve environment-aware database name
    resolved_db = get_database_name(db_name)
...
def store_SQL_data_Insert_Ignore(db, sql_table_name, data, _raise=False):
    ...
    with engine.begin() as connection:
        ...
        try:
            data.to_sql(
                "temp", con=connection, if_exists="append", index=False, chunksize=1000
            )
...
def dynamic_batch_update(db, table_name, update_values, condition, debug=False):
    """
    ...
    where_clause = " AND ".join([f"{col} = :cond_{col}" for col in condition.keys()])
...
def _rewrite_query_database_names(query: str, old_db: str, new_db: str) -> str:
    """
    Rewrite database references in SQL queries (case-insensitive).
    ...
    Handles case-insensitive matching (e.g., PORTFOLIO_DATA vs portfolio_data).
    ...
    """
    # Replace database.table references (case-insensitive)
    # Match old_db.table in any case combination
...
class DatabaseAdapter:
    ...
    def save_to_database(
        ...
        """
        # Resolve environment-aware database name
        resolved_db = get_database_name(db)
