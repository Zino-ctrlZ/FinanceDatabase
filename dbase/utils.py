...
def default_timestamp(index):
    ...
    index = pd.DatetimeIndex([x.replace(hour=0, minute=0, second=0) for x in index], name=idx_name, freq=idx_freq)
    return index
...
def enforce_bus_hours(series: pd.Series|pd.DataFrame) -> pd.DatetimeIndex:
    ...
    """
    bus_hourse = series.index.indexer_between_time(PRICING_CONFIG['MARKET_OPEN_TIME'], PRICING_CONFIG['MARKET_CLOSE_TIME'])
    if len(bus_hourse) == 0:
        return series
    else:
...
def bus_range(start: str, end: str, freq: str, filter_holiday: bool = True) -> pd.DatetimeIndex:
    """
    ...
    _range = [x for x in _range if x.date().strftime("%Y-%m-%d") not in HOLIDAY_SET] if filter_holiday else _range
